...
import { BookmarksSqLiteRepository } from './bookmarks/BookmarksSqLiteRepository'
...
export class Application {
    public static async createApplication() {
        //TODO improve injections and remove Cat* code
        const catService = new CatService(new CatRepository())
        const requestServices = { catService }
        const expressServer = new ExpressServer(new CatEndpoints(), requestServices, new ReleaseNotesController(), new BookmarksController())

        await expressServer.setup(Environment.getPort())
        Application.handleExit(expressServer)
        //DDBB
        const bookmarksSqLiteRepository = new BookmarksSqLiteRepository()
        await BookmarksSqLiteRepository.open()
        // return the Express
