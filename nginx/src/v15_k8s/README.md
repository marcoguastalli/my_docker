...
docker build --no-cache -t simple-nginx:v1 .
docker login
docker push marco27/simple-nginx:v1

### pull
docker pull marco27/simple-nginx:v1

### deploy
cd ~/my_docker/nginx/src/v15_k8s
...
### folder
kubectl apply -f nginx_v15_k8s/ --namespace ingress-nginx
kubectl delete -f nginx_v15_k8s/ --namespace ingress-nginx
